  ...
  <div class="page dark-mode-transition">
    ...
    <div class="filter-section">
      <div class="filter-header">
        <div class="filter-title">
          ...
          <p>Tìm kiếm và lọc CPU theo tiêu chí</p>
        </div>
      ...
      <div class="filter-row">
        <div class="filter-group">
          ...
          <input 
            type="text" 
            v-model="searchText" 
            placeholder="Tìm theo tên CPU, mã CPU..."
            @input="applyFilters"
            class="search-input"
          />
        </div>
    ...
    <div class="add-customer-section">
      <div class="add-buttons">
        ...
        <button class="btn-add-customer" @click="addCpu()">
    ...
    <div class="main-content">
      ...
      <div class="table-header">
        <div class="table-title">
          <h2>Danh sách CPU</h2>
          <span class="item-count">{{ totalItems }} CPU</span>
        </div>
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            <tr v-for="(cpu, index) in paginatedCpus" :key="cpu.id">
              <td class="checkbox-column">
                <input 
                  type="checkbox" 
                  class="row-checkbox" 
                  :value="cpu.id"
                  :checked="selectedCpuIds.includes(cpu.id)"
                  @change="toggleSelectCpu(cpu.id)"
                />
              </td>
              ...
              <td>
                <div class="customer-info">
                  <div class="customer-avatar cpu-avatar">
                    <FontAwesomeIcon :icon="['fas', 'microchip']" />
                  </div>
                  <div class="customer-details">
                    <div class="customer-name">{{ cpu.tenCpu }}</div>
                    <div class="customer-email">{{ cpu.moTa || 'Chưa có mô tả' }}</div>
                  </div>
                </div>
              </td>
              <td class="address-cell">
                <div class="address-content" :title="cpu.moTa || '-'">
                  {{ cpu.moTa || '-' }}
                </div>
              </td>
              <td>
                <span class="status-badge" :class="cpu.trangThai === 1 ? 'badge-active' : 'badge-inactive'">
                  {{ cpu.trangThai === 1 ? 'Hoạt động' : 'Ngừng hoạt động' }}
                  </span>
              </td>
              <td>
                <div class="action-buttons">
                  <div class="button-row">
                    <button class="btn-view" @click="viewCpu(cpu)" title="Xem chi tiết">
                      <font-awesome-icon icon="eye" />
                  </button>
                    <button class="btn-edit" @click="editCpu(cpu)" title="Chỉnh sửa">
                      <font-awesome-icon icon="edit" />
                  </button>
                  </div>
                  <div class="toggle-container">
                    <label class="toggle-switch" :title="cpu.trangThai === 1 ? 'Gạt để vô hiệu hóa' : 'Gạt để kích hoạt'">
                      <input 
                        type="checkbox"
                        :checked="cpu.trangThai === 1"
                        @change="toggleCpuStatus(cpu)"
                      />
      ...
      <div class="enhanced-modal">
        <div class="modal-header">
          <div class="modal-title">
            <h3>{{ editingCpu ? 'Sửa CPU' : 'Thêm CPU Mới' }}</h3>
          </div>
        ...
        <div class="modal-body">
            ...
            <div class="field-group">
              <label class="field-label">Mã CPU *</label>
              <div class="input-with-button">
                <input 
                  v-model="formData.maCpu" 
                  type="text" 
                  placeholder="VD: CPU001"
                  class="form-input"
                  required
                />
                <button @click="generateMaCpuAuto" class="btn-generate" title="Tự động tạo mã">
            ...
            <div class="field-group">
              <label class="field-label">Tên CPU *</label>
              <input 
                v-model="formData.tenCpu" 
                type="text" 
...
const cpus = ref<Cpu[]>([])
const selectedCpus = ref<Cpu[]>([])
const selectedCpuIds = ref<number[]>([])
...
const allFilteredCpus = computed(() => {
  ...
  console.log('=== FILTERING CPUS ===')
  console.log('Original cpus count:', cpus.value.length)
  ...
  if (statusFilter.value !== '') {
    ...
    console.log('No status filter applied - showing all cpus')
  }

  // Filter by brand (based on CPU name first letter)
  if (brandFilter.value) {
    filtered = filtered.filter(cpu => {
      const name = cpu.tenCpu.toLowerCase()
      const brand = brandFilter.value.toLowerCase()
      
      if (brand === 'i') {
        return name.startsWith('intel') || name.includes('core')
      } else if (brand === 'a') {
        return name.startsWith('amd') || name.includes('ryzen')
      } else if (brand === 's') {
        return name.startsWith('snapdragon') || name.includes('qualcomm')
      } else if (brand === 'm') {
        return name.startsWith('mediatek') || name.includes('helio')
      } else if (brand === 'o') {
        return !name.startsWith('intel') && !name.startsWith('amd') && 
               !name.startsWith('snapdragon') && !name.startsWith('mediatek')
      }
      return false
    })
  }
  ...
  if (searchText.value) {
    ...
    filtered = filtered.filter(cpu => 
      cpu.tenCpu?.toLowerCase().includes(search) ||
      cpu.maCpu?.toLowerCase().includes(search) ||
      cpu.moTa?.toLowerCase().includes(search)
    )
  }

  totalItems.value = filtered.length
  console.log('Final filtered cpus count:', filtered.length)
  console.log('=== END FILTERING ===')
  return filtered
})

// Computed property for paginated cpus
const paginatedCpus = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value
  const end = start + itemsPerPage.value
  return allFilteredCpus.value.slice(start, end)
})
...
// Computed property for selected cpus
const selectedCpusComputed = computed(() => {
  return cpus.value.filter(cpu => selectedCpuIds.value.includes(cpu.id))
})

// Computed property for select all checkbox
const isAllSelected = computed(() => {
  return paginatedCpus.value.length > 0 && 
         paginatedCpus.value.every(cpu => selectedCpuIds.value.includes(cpu.id))
})
...
async function loadCpus() {
  try {
    loading.value = true
    const response = await api.get('/api/cpu')
    const allCpus = response.data || []
    
    // Sort cpus by creation date (newest first)
    cpus.value = allCpus.sort((a: any, b: any) => {
      const dateA = new Date(a.ngayTao || a.ngayCapNhat || 0)
      const dateB = new Date(b.ngayTao || b.ngayCapNhat || 0)
      return dateB.getTime() - dateA.getTime() // Newest first
    })
    
    // Debug: Log all cpus and their status
    console.log('=== LOADED CPUS ===')
    console.log('Total cpus loaded:', cpus.value.length)
    cpus.value.forEach((cpu, index) => {
...
function showInactiveCpus() {
  statusFilter.value = '0'
  currentPage.value = 1
  toastRef.value?.info('Thông báo', 'Đang hiển thị CPU ngừng hoạt động. Gạt toggle để kích hoạt lại!')
}
...
// Test function to create a cpu with sample data
function testCreateCpu() {
  formData.value = {
    maCpu: 'CPUTEST001',
    tenCpu: 'CPU Test',
    moTa: 'CPU test để kiểm tra API',
    trangThai: 1
  }
...
async function toggleCpuStatus(cpu: Cpu) {
  try {
    const newStatus = cpu.trangThai === 1 ? 0 : 1
    
    console.log('=== TOGGLE CPU STATUS ===')
    console.log('Cpu ID:', cpu.id)
    console.log('Cpu name:', cpu.tenCpu)
    console.log('Current status:', cpu.trangThai)
    console.log('New status:', newStatus)
    
    // Use the new simple endpoint - ensure clean ID
    const cleanId = parseInt(String(cpu.id).replace(/[^\d]/g, ''), 10)
    console.log('Clean ID for API call:', cleanId)
    
    const response = await api.put(`/api/cpu/${cleanId}/status`, {
      trangThai: newStatus
    })
    ...
    // Update local state
    cpu.trangThai = newStatus
    
    if (newStatus === 0) {
      toastRef.value?.success('Thành công', 'Đã vô hiệu hóa CPU!')
    } else {
      toastRef.value?.success('Thành công', 'Đã kích hoạt CPU!')
    }
  } catch (error) {
    console.error('Lỗi khi cập nhật trạng thái:', error)
    console.error('Error details:', error.response?.data)
    toastRef.value?.error('Lỗi', 'Không thể cập nhật trạng thái CPU')
  }
}
...
function toggleSelectAll() {
  if (isAllSelected.value) {
    // Unselect all cpus on current page
    ...
    // Select all cpus on current page
    const currentPageIds = paginatedCpus.value.map(c => c.id)
...
function toggleSelectCpu(cpuId: number) {
  const index = selectedCpuIds.value.indexOf(cpuId)
  if (index > -1) {
    selectedCpuIds.value.splice(index, 1)
  } else {
    selectedCpuIds.value.push(cpuId)
  }
}

function exportToExcel() {
  try {
    // Get selected cpus or all cpus
    const cpusToExport = selectedCpusComputed.value.length > 0 ? selectedCpusComputed.value : cpus.value
    
    if (cpusToExport.length === 0) {
      toastRef.value?.warning('Cảnh báo', 'Không có dữ liệu CPU để xuất Excel')
      return
    }
    
    // Tạo dữ liệu cho Excel
    const excelData = cpusToExport.map((cpu, index) => ({
      'STT': index + 1,
      'Mã CPU': cpu.maCpu,
      'Tên CPU': cpu.tenCpu,
      'Mô tả': cpu.moTa || '',
      'Trạng Thái': cpu.trangThai === 1 ? 'Hoạt động' : 'Ngừng hoạt động',
      'Ngày Tạo': cpu.ngayTao ? new Date(cpu.ngayTao).toLocaleDateString('vi-VN') : '',
      'Ngày Cập Nhật': cpu.ngayCapNhat ? new Date(cpu.ngayCapNhat).toLocaleDateString('vi-VN') : ''
    }))
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Danh sách CPU')
    
    // Xuất file Excel
    const fileName = `danh_sach_cpu_${new Date().toISOString().split('T')[0]}.xlsx`
    XLSX.writeFile(wb, fileName)

    toastRef.value?.success('Thành công', 'Đã export danh sách CPU thành công!')
  } catch (error) {
...
// Function to generate maCpu automatically
function generateMaCpuAuto() {
  if (formData.value.tenCpu) {
    const words = formData.value.tenCpu.toLowerCase().split(' ')
    const prefix = words.map(word => word.charAt(0)).join('').toUpperCase()
    const timestamp = Date.now().toString().slice(-3)
    formData.value.maCpu = `CPU${prefix}${timestamp}`
    } else {
    // Generate random code if no name
    const randomCode = Math.random().toString(36).substring(2, 5).toUpperCase()
    const timestamp = Date.now().toString().slice(-3)
    formData.value.maCpu = `CPU${randomCode}${timestamp}`
  }
}

function handleFormSubmit() {
  // Validate form data
  if (!formData.value.maCpu || !formData.value.tenCpu) {
    toastRef.value?.error('Lỗi', 'Vui lòng điền đầy đủ thông tin bắt buộc')
    return
  }
  
  if (editingCpu.value) {
    updateCpu(editingCpu.value.id, formData.value)
  } else {
    createCpu(formData.value)
  }
}

async function createCpu(formData: any) {
  try {
    console.log('Creating cpu with data:', formData)
    const response = await api.post('/api/cpu', formData)
    console.log('API response:', response.data)
    await loadCpus()
    showForm.value = false
    resetFormData()
    toastRef.value?.success('Thành công', 'Đã thêm CPU mới')
  } catch (error) {
    console.error('Error creating cpu:', error)
    console.error('Error response:', error.response?.data)
    toastRef.value?.error('Lỗi tạo CPU', 'Không thể tạo CPU mới')
  }
}

async function updateCpu(id: number, formData: any) {
  try {
    await api.put(`/api/cpu/${id}`, formData)
    await loadCpus()
    showForm.value = false
    toastRef.value?.success('Thành công', 'Đã cập nhật CPU')
  } catch (error) {
