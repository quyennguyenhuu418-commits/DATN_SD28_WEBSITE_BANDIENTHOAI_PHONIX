          ...
          <div class="product-selection-section">
            ...
            <div class="products-table-container">
              <table class="products-table">
                ...
                <tbody>
                  <tr v-for="product in filteredProducts" :key="product.idSanPham" class="product-row">
                    <td>
                      <input
                        type="checkbox"
                        :checked="isProductSelected(product.idSanPham)"
                    ...
                    <td class="product-category">{{ product.tenDanhMuc }}</td>
...
async function loadAppliedProducts() {
  ...
  try {
    ...
    const appliedProductIds = data.map((item: any) => item.idSanPham)
    console.log('🆔 Applied product IDs from API:', appliedProductIds)
    console.log('📦 Available products:', productList.value.length, 'items')
    
    selectedProducts.value = productList.value.filter(p => appliedProductIds.includes(p.idSanPham))
...
async function saveAppliedProducts(khuyenMaiId: number) {
  try {
    ...
    const idSanPhams = selectedProducts.value.map(p => p.idSanPham)
    console.log('🆔 Product IDs to save:', idSanPhams)
...
const filteredProducts = computed(() => {
  ...
  if (productCategoryFilter.value) {
    filtered = filtered.filter(p => p.danhMucId === parseInt(productCategoryFilter.value))
  }

  // Brand filter
  if (productBrandFilter.value) {
    filtered = filtered.filter(p => p.hangId === parseInt(productBrandFilter.value))
  }
...
function isProductSelected(productId: number): boolean {
  return selectedProducts.value.some(p => p.idSanPham === productId)
}
...
function toggleProductSelection(product: any) {
  ...
  const index = selectedProducts.value.findIndex(p => p.idSanPham === product.idSanPham)
...
function toggleAllProducts() {
  ...
  if (allProductsSelected.value) {
    ...
    const filteredIds = filteredProducts.value.map(p => p.idSanPham)
    console.log('🆔 Filtered IDs to remove:', filteredIds)
    
    selectedProducts.value = selectedProducts.value.filter(p => !filteredIds.includes(p.idSanPham))
    console.log('➖ Unselected all. New count:', selectedProducts.value.length)
  } else {
    // Select all filtered products
    const newSelections = filteredProducts.value.filter(p => !isProductSelected(p.idSanPham))
