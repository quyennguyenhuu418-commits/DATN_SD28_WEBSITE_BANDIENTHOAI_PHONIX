    ...
    <div class="filter-section">
      <div class="filter-header">
        ...
        <div class="filter-actions">
          <button class="btn-show-inactive" @click="showInactivePhanCa">
            Xem phân ca ngừng hoạt động
          </button>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(phanCa, index) in filteredPhanCa" :key="phanCa.id">
              ...
              <td>{{ phanCa.nhanVien?.hoTen || '-' }}</td>
              <td>{{ phanCa.ca?.tenCa || '-' }}</td>
              ...
              <td>
                <div class="action-buttons">
                  <div class="button-row">
                    ...
                    <button class="btn-edit" @click="editPhanCa(phanCa)" title="Chỉnh sửa">
                  ...
                  <div class="toggle-container">
                    <label class="toggle-switch">
                      <input 
                        type="checkbox"
                        :checked="phanCa.trangThai === 1"
                        @change="toggleStatus(phanCa.id)"
                      />
...
// Computed property for filtered phanCa (without pagination)
const allFilteredPhanCa = computed(() => {
  let filtered = phanCaList.value

  // Filter by nhanVien
  if (nhanVienFilter.value !== '') {
    filtered = filtered.filter(p => p.nhanVienId.toString() === nhanVienFilter.value)
  }

  // Filter by ca
  if (caFilter.value !== '') {
    filtered = filtered.filter(p => p.caId.toString() === caFilter.value)
  }

  // Filter by date range
  if (fromDateFilter.value !== '') {
    filtered = filtered.filter(p => p.ngayLamViec >= fromDateFilter.value)
  }

  if (toDateFilter.value !== '') {
    filtered = filtered.filter(p => p.ngayLamViec <= toDateFilter.value)
  }
  ...
  if (searchText.value.trim() !== '') {
    ...
    filtered = filtered.filter(p => 
      (p.nhanVien?.hoTen && p.nhanVien.hoTen.toLowerCase().includes(search)) ||
      (p.ca?.tenCa && p.ca.tenCa.toLowerCase().includes(search)) ||
      (p.ghiChu && p.ghiChu.toLowerCase().includes(search))
    )
  }
...
// Computed property for paginated phanCa
...
const isAllSelected = computed(() => {
  return filteredPhanCa.value.length > 0 && 
         filteredPhanCa.value.every(phanCa => selectedPhanCaIds.value.includes(phanCa.id))
})

// Computed property for selected phanCa
const selectedPhanCaComputed = computed(() => {
  return phanCaList.value.filter(phanCa => selectedPhanCaIds.value.includes(phanCa.id))
})
...
async function loadPhanCa() {
  loading.value = true
  try {
    console.log('Loading phanCa from /api/phan-ca...')
    const { data } = await api.get<PhanCa[]>('/api/phan-ca')
    console.log('Loaded phanCa:', data)
...
async function toggleStatus(id: number) {
  try {
    await api.post(`/api/phan-ca/${id}/toggle-status`)
    
    // Cập nhật trạng thái trong danh sách
    const phanCa = phanCaList.value.find(p => p.id === id)
    if (phanCa) {
      phanCa.trangThai = phanCa.trangThai === 1 ? 0 : 1
    }
    
    if (phanCa?.trangThai === 0) {
      toastRef.value?.success('Thành công', 'Đã vô hiệu hóa phân ca!')
    } else {
      toastRef.value?.success('Thành công', 'Đã kích hoạt phân ca!')
    }
  } catch (error: any) {
...
function showInactivePhanCa() {
  // This would need to be implemented based on your status logic
  currentPage.value = 1
  toastRef.value?.info('Thông báo', 'Đang hiển thị phân ca ngừng hoạt động')
}
...
function getStatusText(phanCa: PhanCa): string {
  return phanCa.trangThai === 1 ? 'Hoạt động' : 'Đã kết thúc'
}

function getStatusBadgeClass(phanCa: PhanCa): string {
  return phanCa.trangThai === 1 ? 'badge-active' : 'badge-inactive'
}
...
function exportToExcel() {
  try {
    ...
    const excelData = phanCaToExport.map((phanCa, index) => ({
      'STT': index + 1,
      'Nhân viên': phanCa.nhanVien?.hoTen || '',
      'Ca làm việc': phanCa.ca?.tenCa || '',
      'Ngày làm việc': formatDate(phanCa.ngayLamViec),
      'Ghi chú': phanCa.ghiChu || '',
      'Trạng thái': getStatusText(phanCa)
    }))
...
function toggleSelectAll() {
  if (isAllSelected.value) {
    const currentPageIds = filteredPhanCa.value.map(p => p.id)
