...
public class HoaDonServiceImpl implements HoaDonService {
    ...
    public OnlineOrderResponse createOnlineOrder(OnlineOrderRequest request) {
        try {
            // Create the main invoice
            HoaDon hoaDon = new HoaDon();
            hoaDon.setKhachHangId(null); // Online order, no customer ID
            hoaDon.setNhanVienId(null); // No staff assigned yet
            hoaDon.setPhieuGiamGiaId(null); // No voucher for now
            hoaDon.setTongTien(request.getTongTien().doubleValue());
            ...
            hoaDon.setPhiVanChuyen(request.getPhiVanChuyen().doubleValue());
            hoaDon.setTongTienSauGiam(request.getTongTien().doubleValue());
            hoaDon.setLoaiHoaDon(request.getLoaiHoaDon() != null ? request.getLoaiHoaDon() : "ONLINE");
            ...
            if (request.getChiTietDonHang() != null && !request.getChiTietDonHang().isEmpty()) {
                for (OnlineOrderRequest.ChiTietDonHangRequest chiTiet : request.getChiTietDonHang()) {
                    HoaDonCt hoaDonCt = new HoaDonCt();
                    hoaDonCt.setHoaDon(savedHoaDon);

                    // Get ChiTietSanPham
                    ChiTietSanPham chiTietSanPham = chiTietSanPhamRepository.findById(chiTiet.getChiTietSanPhamId())
                            .orElseThrow(() -> new RuntimeException("Chi tiết sản phẩm không tồn tại: " + chiTiet.getChiTietSanPhamId()));
                    hoaDonCt.setChiTietSanPham(chiTietSanPham);

                    hoaDonCt.setDonGia(chiTiet.getGia());
                    hoaDonCt.setThanhTien(chiTiet.getThanhTien());
                    hoaDonCt.setTrangThai(1); // Active
                    ...
                    int newQuantity = currentQuantity - chiTiet.getSoLuong();
                    chiTietSanPham.setSoLuong(newQuantity);
                    chiTietSanPhamRepository.save(chiTietSanPham);
                }
            }
            ...
            e.printStackTrace();
            throw new RuntimeException("Lỗi khi tạo đơn hàng online: " + e.getMessage());
        }
    }
...
//        dto.setPhuongThucThanhToan(hoaDon.getPhuongThucThanhToan());
    ...
    private List<com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO> getLichSuTrangThai(Integer hoaDonId) {
        try {
            ...
            List<com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO> result = trackingList.stream()
                    .map(this::convertToTrangThaiTrackingDto)
                    .collect(Collectors.toList());
            
            System.out.println("🔍 Converted to " + result.size() + " DTOs");
            for (com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO dto : result) {
    ...
    private com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO convertToTrangThaiTrackingDto(TrangThaiTracking tracking) {
        com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO dto = new com.example.datn_sd28_2025.dto.TrangThaiTrackingDTO();
    ...
    private List<com.example.datn_sd28_2025.dto.SanPhamTrackingDTO> getSanPhamTrackingList(Integer hoaDonId) {
        List<com.example.datn_sd28_2025.dto.SanPhamTrackingDTO> products = new ArrayList<>();
        ...
        try {
            ...
            for (HoaDonCt hoaDonChiTiet : hoaDonChiTiets) {
                ...
                if (chiTietSanPham != null) {
                    ...
                    if (sanPham != null) {
                        com.example.datn_sd28_2025.dto.SanPhamTrackingDTO product = 
                            new com.example.datn_sd28_2025.dto.SanPhamTrackingDTO();
                        ...
                        if (chiTietSanPham.getHinhAnhs() != null && !chiTietSanPham.getHinhAnhs().isEmpty()) {
                            product.setHinhAnh(chiTietSanPham.getHinhAnhs().get(0).getUrlAnh());
                        } else {
                            product.setHinhAnh("/placeholder-product.png");
                        }
            ...
            System.err.println("Error loading product tracking list: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public HoaDonDTO updateTrangThaiByMaHoaDon(String maHoaDon, Integer trangThai) {
        try {
            ...
            System.out.println("🔍 Checking status change: oldTrangThai=" + oldTrangThai + ", newTrangThai=" + trangThai);
            if (!oldTrangThai.equals(trangThai)) {
                String tenTrangThai = OrderStatusUtil.getStatusName(trangThai);
                String moTa = "Trạng thái thay đổi từ " + OrderStatusUtil.getStatusName(oldTrangThai) + " sang " + tenTrangThai;
                String nguoiThucHien = "Hệ thống"; // Có thể lấy từ context security sau này
                ...
                System.out.println("🔍 Saving tracking: " + tracking);
                TrangThaiTracking savedTracking = trangThaiTrackingRepository.save(tracking);
                System.out.println("✅ Đã lưu lịch sử trạng thái: ID=" + savedTracking.getId() + ", " + tenTrangThai + " cho hóa đơn " + maHoaDon + " tại " + savedTracking.getThoiGian());
            } else {
                System.out.println("⚠️ Trạng thái không thay đổi, không lưu lịch sử");
            }
