...
public class GiaoCaController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<GiaoCaDTO> update(@PathVariable Integer id, @RequestBody GiaoCaDTO giaoCaDTO) {
        try {
            // Bỏ @Valid để cho phép update mà không cần các field required như soTienDauCa
            // Vì các field readonly không được gửi từ frontend
            System.out.println("[GiaoCaController] Updating giao ca ID: " + id + " with data: " + giaoCaDTO);
            GiaoCaDTO updatedGiaoCa = giaoCaService.update(id, giaoCaDTO);
            System.out.println("[GiaoCaController] Giao ca updated successfully: " + updatedGiaoCa.getId());
            return ResponseEntity.ok(updatedGiaoCa);
        } catch (RuntimeException e) {
            System.out.println("[GiaoCaController] Runtime error updating giao ca: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.badRequest().body(null);
        } catch (Exception e) {
            System.out.println("[GiaoCaController] Unexpected error updating giao ca: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
