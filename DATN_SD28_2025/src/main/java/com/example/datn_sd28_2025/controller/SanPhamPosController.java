...
public class SanPhamPosController {
    ...
    public ResponseEntity<Map<String, Object>> debugSales() {
        Map<String, Object> result = new HashMap<>();

        try {
            // Đếm tổng ImeiDaBan
            Long totalImeiDaBan = hoaDonCtRepository.countAllImeiDaBan();
            result.put("totalImeiDaBan", totalImeiDaBan);

            // Lấy danh sách sản phẩm với soLuongDaBan
            List<SanPhamPosDTO> allProducts = sanPhamPosService.getAllProductsForPos();

            // Test với tất cả sản phẩm iPhone 20
            List<SanPhamPosDTO> iphone20Products = allProducts.stream()
                    .filter(p -> p.getTenSanPham().toLowerCase().contains("iphone 20"))
                    .collect(java.util.stream.Collectors.toList());

            result.put("iphone20Products", iphone20Products);

            // Test với tất cả iPhone 20 products
            for (SanPhamPosDTO iphone20 : iphone20Products) {
                Long salesCount = hoaDonCtRepository.countSoLuongDaBanByChiTietSanPhamId(iphone20.getChiTietSanPhamId());
                result.put("iphone20_" + iphone20.getChiTietSanPhamId() + "_sales", salesCount);
                result.put("iphone20_" + iphone20.getChiTietSanPhamId() + "_name", iphone20.getTenSanPham());
            }
            result.put("totalProducts", allProducts.size());
            result.put("products", allProducts.stream().limit(5).collect(java.util.stream.Collectors.toList()));
