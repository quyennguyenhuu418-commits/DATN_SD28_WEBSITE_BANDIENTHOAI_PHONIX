  ...
  <header class="pos-header">
    ...
    <div class="header-right">
      ...
      <div class="language-switcher-container">
        <button class="language-switcher-btn" @click="toggleLanguageSwitcher" :title="t('header.language')">
        ...
        <!-- Language Dropdown -->
        <div v-if="showLanguageSwitcher" class="language-dropdown">
          <div v-for="lang in availableLanguages" :key="lang.code"
            ...
            <svg v-if="getCurrentLanguage() === lang.code" class="w-4 h-4 check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const { t, setLanguage, getCurrentLanguage, getAvailableLanguages } = useGlobalTranslation()
...
const handleClickOutside = (event) => {
  ...
  if (showLanguageSwitcher.value && !event.target.closest('.language-switcher-container')) {
    showLanguageSwitcher.value = false
  }
}
...
const updatePageInfo = () => {
  ...
  // 0) If explicit props provided, use them with highest priority
  if ((props.pageTitle && props.pageTitle.length) || (props.pageSubtitle && props.pageSubtitle.length)) {
    pageTitle.value = props.pageTitle || pageTitles[path] || ''
    pageSubtitle.value = props.pageSubtitle || pageSubtitles[path] || ''
    return
  }
  ...
  if (path.startsWith('/san-pham/form')) {
    pageTitle.value = 'Sản phẩm'
    pageSubtitle.value = 'Quản lý sản phẩm'
  } else if (path.startsWith('/voucher/form')) {
...
onMounted(() => {
  ...
  const { initLanguage } = useGlobalTranslation()
  initLanguage()
