<template>
  <div id="ultra-chatbox" class="ultra-chatbox-container">
    ...
    <!-- Chat Window -->
            ...
            >
      <!-- Minimized icon (click to reopen) -->
      <div v-if="isMinimized" class="minimized-icon" title="Mở chatbot" @click.stop="maximizeChat">
...
.ultra-chatbox-container {
  position: fixed;
  bottom: calc(2rem + 55px + 16px); /* SimpleChat (55) + gap 16px */
  right: 2rem;
  z-index: 9999;
...
.ultra-chat-toggle {
  width: 55px;
  height: 55px;
  background: linear-gradient(135deg, #f97316, #ea580c); /* Phoenix orange */
  border-radius: 50%;
  display: flex;
  align-items: center;
...
.ultra-chat-window.minimized {
  ...
  padding: 0;
  overflow: hidden;
  background: linear-gradient(135deg, #f97316, #ea580c); /* màu cam khi thu nhỏ */
  color: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
