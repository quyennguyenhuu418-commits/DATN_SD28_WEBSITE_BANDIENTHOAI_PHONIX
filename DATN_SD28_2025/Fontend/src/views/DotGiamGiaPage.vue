            ...
            <td>
              ...
              <td>
                <div class="product-count">
                  <span v-if="item.soLuongSanPham === 0" class="count-label">Chưa có sản phẩm</span>
...
async function toggleStatus(id: number) {
  ...
  confirmTitle.value = `Xác nhận ${statusText} đợt giảm giá`
  confirmMessage.value = `Bạn có chắc chắn muốn ${statusText} đợt giảm giá "${item.tenKhuyenMai}"?`
...
async function confirmToggleStatus() {
  ...
  try {
    ...
    const statusText = item.trangThai === 1 ? 'vô hiệu hóa' : 'kích hoạt'
    toastRef.value?.success('Thành công', `Đã ${statusText} đợt giảm giá thành công!`)
...
function exportToExcel() {
  try {
    ...
    'Số lượng sản phẩm': item.soLuongSanPham !== undefined ? (item.soLuongSanPham === 0 ? 'Chưa có sản phẩm' : item.soLuongSanPham) : '-',
