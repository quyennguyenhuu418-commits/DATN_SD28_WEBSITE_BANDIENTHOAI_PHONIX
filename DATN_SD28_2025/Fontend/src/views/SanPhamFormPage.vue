...
function getColorCode(idMauSac: number | null) {
  ...
  if (mauSac) {
    // Sử dụng ma_hex từ database
    if (mauSac.maHex) {
      return mauSac.maHex.startsWith('#') ? mauSac.maHex : `#${mauSac.maHex}`
    }
    
    // Fallback: sử dụng maMau nếu có
    if (mauSac.maMau) {
    ...
    if (isCreate.value) {
      ...
      const payload = {
        ...
        variants: variants.value.map(v => ({
          ...
          ghiChu: v.ghiChu || '',
          imeis: v.imeis || [],
          imageUrls: colorImages.value[v.idMauSac] || []
        }))
      }
...
async function loadProductData() {
  try {
    ...
    if (productData.variants && productData.variants.length > 0) {
      ...
      // Load images into colorImages
      variants.value.forEach(variant => {
        if (variant.idMauSac && variant.imageUrls && variant.imageUrls.length > 0) {
          colorImages.value[variant.idMauSac] = [...variant.imageUrls]
        }
      })
    }
  ...
  <div class="page">
    ...
    <div class="page-header">
      ...
      <button class="btn-refresh" @click="clearDraftAndRefresh">
        <FontAwesomeIcon icon="refresh" class="btn-icon" />
        Làm mới
      </button>
    </div>
              ...
              <div class="multi-select-content">
                <div v-if="variantForm.selectedMauSacs.length > 0" class="selected-colors">
                  <div v-for="mauSacId in variantForm.selectedMauSacs" :key="mauSacId" class="selected-color-item">
                    <div class="color-indicator" :class="`color-${mauSacId}`" :style="{ backgroundColor: getColorCode(mauSacId) }"></div>
                    <span>{{ getAttributeName(mauSacId, mauSacs, 'tenMau') }}</span>
                  </div>
                </div>
                <span v-else class="multi-select-text">Chọn Màu sắc</span>
              </div>
              <span class="multi-select-arrow">▼</span>
            </div>
          ...
          <div class="variant-group-header">
            ...
            <div class="price-input-section">
              <input
                ...
                type="number"
              />
              <button class="btn-apply-price" @click="applyPriceToAll(group.key)"><FontAwesomeIcon icon="check" class="btn-icon btn-icon--md" /> Áp dụng</button>
              ...
              <tr v-for="(v, i) in group.variants" :key="i" class="variant-row">
                ...
                <td>
                  <div class="variant-color">
                    <div class="color-indicator" :class="`color-${v.idMauSac}`" :style="{ backgroundColor: getColorCode(v.idMauSac) }"></div>
                    <span>{{ getAttributeName(v.idMauSac, mauSacs, 'tenMau') }}</span>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div class="variant-actions">
                    <button class="btn-delete" @click="removeVariantFromGroup(group.key, i)">
                        <FontAwesomeIcon icon="trash" class="btn-icon btn-icon--md" />
                    </button>
                    <input
                      type="file"
                      ...
                      style="display: none"
                    />
                    <button class="btn-import-excel" @click="importFromExcel(v.originalIndex)"><FontAwesomeIcon icon="file-import" class="btn-icon btn-icon--md" /> Nhập</button>
                  </div>
                </td>
              </tr>
              </tbody>
...
.page { 
  padding: 20px; 
  margin-top: 90px; /* Space for fixed PosHeader - Test */
}
.page-header {
  display: flex;
...
/* Button Icon Styles */
.btn-icon { width: 18px; height: 18px; font-size: 16px; margin-right: 6px; vertical-align: middle; color: inherit; }
.btn-icon--md { width: 18px; height: 18px; font-size: 16px; }
.btn-icon--lg { width: 20px; height: 20px; font-size: 18px; }
.btn-secondary {
...
.select-with-add {
  display: flex;
  align-items: stretch; /* ensure same height for children */
  width: 100%;
  gap: 8px;
  min-width: 0;
  flex: 1;
...
.select-with-add select {
  ...
  min-width: 0;
  margin-right: 0;
  height: 48px; /* same as .btn-add */
  border-radius: 10px; /* same curve */
  border: 2px solid #e5e7eb;
  padding: 14px 16px;
  font-size: 14px;
  background: #ffffff;
...
.color-title-content .color-indicator {
  width: 28px;
  height: 28px;
  border-radius: 50%;
...
.color-title-content:hover .color-indicator {
  transform: scale(1.1);
  box-shadow: 0 6px 12px rgba(0, 0, 0, 0.2);
}

.color-title-content span {
  color: #1e293b;
  font-size: 16px;
  font-weight: 600;
}
...
.variants-table th:nth-child(7), .variants-table td:nth-child(7) { width: 100px; min-width: 80px; text-align: center;}
...
.color-option:hover .color-indicator {
  transform: scale(1.1);
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);
}
...
.selected-color-item .color-indicator {
  width: 16px;
  height: 16px;
  border-radius: 50%;
  border: 1px solid #ddd;
...
.variant-color:hover .color-indicator {
  transform: scale(1.1);
  box-shadow: 0 4px 8px rgba(0,0,0,0.15);
}
