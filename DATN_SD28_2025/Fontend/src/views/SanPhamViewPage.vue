            ...
            <tr v-for="(ct, index) in paginatedChiTiets" :key="ct.id" class="variant-row" :class="{ selected: selectedVariant?.id === ct.id }" @click="selectedVariant = ct">
              ...
              <td>
                <div class="text-right">
                  <div v-if="getGiaGoc(ct) > getGiaBan(ct)" class="text-xs line-through text-slate-400">
                    {{ formatPrice(getGiaGoc(ct)) }}
                  </div>
                  <div class="font-semibold">{{ formatPrice(getGiaBan(ct)) }}</div>
                  <div v-if="getGiamPhanTram(ct)" class="text-xs text-green-600">-{{ Math.round(getGiamPhanTram(ct) as number) }}%</div>
                </div>
              </td>
              <td>
                <div class="text-xs" :class="getGiamPhanTram(ct) ? 'text-green-600' : 'text-slate-400'">
                  <span v-if="getGiamPhanTram(ct)">{{ Math.round(getGiamPhanTram(ct) as number) }}%</span>
                  <span v-else>—</span>
                </div>
              </td>
      ...
      <div class="variant-detail-modal" @click.stop>
        ...
        <div class="modal-body" v-if="selectedVariant">
          <div class="variant-info-grid">
            <!-- Images Section -->
            <div class="info-section" v-if="variantImages.length > 0">
              <h4 class="section-title">Hình ảnh sản phẩm</h4>
              <div class="images-container">
                <div v-for="(image, index) in variantImages" :key="image.id" class="image-item">
                  <img 
                    :src="image.urlAnh" 
                    :alt="`Hình ảnh ${index + 1}`"
                    class="product-image"
                    @error="handleImageError"
                    @click="openImageModal(image.urlAnh)"
                  />
                </div>
              </div>
            </div>
                    ...
                    <div v-else class="imei-list">
                      <div class="imei-item" v-for="(imei, index) in variantImeis" :key="imei.id">
                        <div class="imei-number">
                          ...
                          <span class="imei-value">{{ imei.imei }}</span>
                        </div>
            ...
            <div class="info-section">
              ...
              <div class="info-grid">
                ...
                <div class="info-item">
                  <label>Giá gốc:</label>
                  <span class="info-value price-value" :class="getGiaGoc(selectedVariant) > getGiaBan(selectedVariant) ? 'line-through text-slate-400' : ''">
                    {{ formatPrice(getGiaGoc(selectedVariant)) }}
                  </span>
                </div>
                <div class="info-item" v-if="getGiamPhanTram(selectedVariant)">
                  <label>Đợt giảm:</label>
                  <span class="info-value">
                    -{{ Math.round(getGiamPhanTram(selectedVariant) as number) }}% {{ selectedVariant.tenDotGiam ? `(${selectedVariant.tenDotGiam})` : '' }}
                  </span>
                </div>
                <div class="info-item">
                  <label>Giá sau đợt:</label>
                  <span class="info-value price-value">{{ formatPrice(getGiaBan(selectedVariant)) }}</span>
                </div>
    ...
    <div v-if="showImageModal" class="modal-overlay" @click="closeImageModal">
      <div class="image-modal" @click.stop>
        ...
        <div class="image-modal-body">
          <img :src="selectedImageUrl" alt="Hình ảnh sản phẩm" class="full-image" />
        </div>
      </div>
    </div>
...
// Enrich current page variants with effective discount pricing from POS API
const enrichedVariantIds = ref<Set<number>>(new Set())
async function enrichPricing(list: any[]) {
  for (const ct of list) {
    const id = Number(ct.id)
    if (!id || enrichedVariantIds.value.has(id)) continue
    try {
      const res = await api.get(`/api/san-pham-pos/${id}`)
      const p = res.data || {}
      if (p) {
        ;(ct as any).giaGoc = p.giaGoc ?? p.gia ?? ct.giaBan ?? ct.donGia
        ;(ct as any).giaSauGiam = p.giaSauGiam ?? p.gia ?? ct.giaBan ?? ct.donGia
        ;(ct as any).giamPhanTram = p.giamPhanTram ?? null
        ;(ct as any).tenDotGiam = p.tenDotGiam ?? null
        ;(ct as any).gia = p.gia ?? ct.giaSauGiam ?? ct.giaBan
      }
      enrichedVariantIds.value.add(id)
    } catch (e) {
      console.warn('Enrich price failed for CTSP', id, e)
    }
  }
}
watch(paginatedChiTiets, async (list) => {
  try { await enrichPricing(list) } catch {}
}, { immediate: true })
...
async function loadChiTiets() {
  try {
    ...
    // Enrich pricing immediately after loading all CTSPs
    enrichedVariantIds.value = new Set()
    await enrichPricing(chiTiets.value)
  } catch (error) {
...
function getGiaBan(ct: any): number {
  const giaSau = (ct?.giaSauGiam ?? ct?.gia ?? ct?.giaBan ?? ct?.donGia ?? 0) as number
  return giaSau
}

function getGiaGoc(ct: any): number {
  const base = (ct?.giaGoc ?? ct?.giaBanGoc ?? ct?.giaBan ?? ct?.donGia ?? ct?.gia ?? 0) as number
  return base
}

function getGiamPhanTram(ct: any): number | null {
  if (ct?.giamPhanTram) return Number(ct.giamPhanTram)
  const goc = getGiaGoc(ct)
  const sau = getGiaBan(ct)
  if (goc > 0 && sau < goc) {
    return Math.max(0, ((goc - sau) / goc) * 100)
  }
  return null
}
...
async function openVariantModal(ct: any) {
  ...
  // Load both IMEIs and images
  await Promise.all([
    loadVariantImeis(ct.id),
    loadVariantImages(ct.id)
  ])
}
...
async function loadVariantImeis(chiTietId: number) {
  try {
    ...
    console.log('📱 API Response for IMEI:', response.data)
    const allImeis = Array.isArray(response.data) ? response.data : []
    console.log('📱 All IMEIs before filter:', allImeis)
    
    // Debug: Xem tất cả IMEI trước khi filter
    console.log('📱 All IMEIs structure:', allImeis)
    
    // Chỉ lấy các IMEI đang hoạt động (trangThai === 1)
    variantImeis.value = allImeis.filter((imei: any) => {
      console.log('📱 IMEI item:', imei, 'trangThai:', imei.trangThai, 'imei:', imei.imei)
      return imei.trangThai === 1
    })
    
    // Nếu sau filter không có IMEI nào, lấy tất cả để debug
    if (variantImeis.value.length === 0 && allImeis.length > 0) {
      console.warn('⚠️ Không có IMEI đang hoạt động, hiển thị tất cả để debug')
      variantImeis.value = allImeis
    }
    console.log('📱 Loaded active IMEIs:', variantImeis.value)
  } catch (error) {
...
async function loadVariantImages(chiTietId: number) {
  try {
    loadingImages.value = true
    const response = await api.get(`/api/san-pham/chi-tiet/${chiTietId}/images`)
    variantImages.value = Array.isArray(response.data) ? response.data : []
    console.log('Loaded images for variant:', chiTietId, variantImages.value)
  } catch (error) {
