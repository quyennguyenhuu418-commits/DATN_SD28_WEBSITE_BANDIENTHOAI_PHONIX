      ...
      <div class="right-panel">
        <div class="login-form-container">
          ...
          <div v-if="step === 2" class="step-content">
            ...
            <div class="otp-info">
              <i class="fas fa-shield-alt"></i>
              <p>Chúng tôi đã gửi mã OTP 5 chữ số đến email:</p>
            ...
            <form @submit.prevent="verifyOtp" class="login-form">
              ...
              <button type="submit" class="btn-login" :disabled="loading || !form.otp || form.otp.length !== 5">
                <i v-if="loading" class="fas fa-spinner fa-spin"></i>
...
const sendOtp = async () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      success.value = response.data.message || 'Mã OTP đã được gửi đến email của bạn!'
      resetToken.value = response.data.resetToken || ''
      step.value = 2
      startResendCooldown()
    } else {
      error.value = response.data?.message || 'Có lỗi xảy ra khi gửi mã OTP'
    }
  } catch (err: any) {
...
const verifyOtp = async () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      success.value = response.data.message || 'OTP đã được xác thực thành công!'
      resetToken.value = response.data.resetToken || resetToken.value
      setTimeout(() => {
        step.value = 3
        success.value = ''
      }, 1500)
    } else {
      error.value = response.data?.message || 'Mã OTP không đúng hoặc đã hết hạn'
    }
  } catch (err: any) {
...
const resetPassword = async () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      ...
      error.value = response.data?.message || 'Có lỗi xảy ra khi đặt lại mật khẩu'
    }
  } catch (err: any) {
...
const resendOtp = async () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      success.value = response.data.message || 'Mã OTP mới đã được gửi!'
      resetToken.value = response.data.resetToken || resetToken.value
      startResendCooldown()
    } else {
      error.value = response.data?.message || 'Có lỗi xảy ra khi gửi lại mã OTP'
    }
  } catch (err: any) {
