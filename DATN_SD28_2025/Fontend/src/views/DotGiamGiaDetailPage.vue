...
async function loadAppliedProducts() {
  ...
  try {
    ...
    console.log('📋 First product details:', appliedProductIds[0])
    console.log('📋 First product keys:', Object.keys(appliedProductIds[0] || {}))
    console.log('📋 First product tenDanhMuc:', appliedProductIds[0]?.tenDanhMuc)

    // Sử dụng trực tiếp dữ liệu từ API khuyến mãi (đã có đầy đủ thông tin)
    appliedProducts.value = appliedProductIds.map((item: any) => ({
      id: item.idChiTietSanPham,
      chiTietSanPhamId: item.idChiTietSanPham,
      tenSanPham: item.tenSanPham,
      maSanPham: item.maSanPham,
      tenHang: item.tenHang,
      tenDanhMuc: item.tenDanhMuc,
      gia: 0, // Không có trong API khuyến mãi
      hinhAnh: null // Không có trong API khuyến mãi
    }))
    ...
    if (appliedProducts.value.length > 0) {
      console.log('📋 First applied product details:', appliedProducts.value[0])
      console.log('📋 First product tenDanhMuc:', appliedProducts.value[0].tenDanhMuc)
      console.log('📋 First product danhMuc:', appliedProducts.value[0].danhMuc)
    }
  } catch (err: any) {
