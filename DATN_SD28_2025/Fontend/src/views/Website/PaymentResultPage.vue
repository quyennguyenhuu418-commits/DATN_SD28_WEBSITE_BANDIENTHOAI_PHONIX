...
onMounted(async () => {
  try {
    ...
    if (paymentResult.value.success) {
      // Clear purchased items from cart (same as COD flow)
      try {
      ...
      const orderRef = paymentResult.value.orderCode || paymentResult.value.orderId || ''
      if (orderRef) {
...
const goToOrderTracking = () => {
  // Redirect customers to public tracking or checkout page, not admin area
  if (paymentResult.value.orderCode) {
    router.push({ path: '/theo-doi-don-hang', query: { orderId: paymentResult.value.orderCode } })
  } else if (paymentResult.value.orderId) {
    router.push({ path: '/theo-doi-don-hang', query: { orderId: paymentResult.value.orderId } })
  } else {
