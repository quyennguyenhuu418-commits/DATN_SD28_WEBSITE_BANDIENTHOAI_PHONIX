<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-white rounded-lg shadow-lg p-6">
      ...
      <div class="space-y-3">
        <button
          @click="isSuccess ? goToOrderTracking : goToCheckout"
          class="w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-700 transition-colors"
        >
          {{ isSuccess ? 'Theo dõi đơn hàng' : 'Thử lại thanh toán' }}
        </button>
...
const goToCheckout = () => {
  // For customers, retry should go back to checkout page
  router.push('/dat-hang')
}

const goToOrderTracking = () => {
  // Redirect customers to public tracking page
  if (transactionDetails.value?.orderCode) {
    router.push({ path: '/theo-doi-don-hang', query: { orderId: transactionDetails.value.orderCode } })
  } else {
    router.push('/dat-hang')
  }
}
...
const handleZaloPayReturn = async () => {
  try {
    ...
    if (result.success) {
      ...
      // Clear purchased items from cart (same as VNPay flow)
      try {
        const stored = sessionStorage.getItem('purchasedItems')
        if (stored) {
          const items = JSON.parse(stored)
          if (Array.isArray(items)) {
            items.forEach(it => { if (it?.chiTietSanPhamId) cartStore.removeItem(it.chiTietSanPhamId) })
          }
        }
      } catch(e) { /* ignore */ }
      ...
      const orderRef = result.orderCode || result.orderId || ''
      if (orderRef) {
        router.replace({ path: '/theo-doi-don-hang', query: { orderId: String(orderRef) } })
        return
      } else {
        // Fallback: redirect to checkout page if no order ID
        router.replace({ path: '/dat-hang', query: { success: '1' } })
        return
      }
    } else {
