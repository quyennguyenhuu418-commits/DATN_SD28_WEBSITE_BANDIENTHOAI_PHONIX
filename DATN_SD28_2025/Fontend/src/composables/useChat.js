...
export function useChat() {
  ...
  const takeCustomer = (customerSessionId) => {
    ...
    if (existingActive) {
      ...
      // Use selectChat to avoid duplicate messages
      selectChat(existingActive)
      return
    }
    ...
    if (customer) {
      ...
      // Use selectChat to properly initialize messages
      selectChat(customer)
  ...
  const selectChat = (chat) => {
    console.log('🎯 selectChat called:', chat.sessionId)
    console.log('📊 Current selectedChat:', selectedChat.value?.sessionId)
    console.log('📊 Current messages count:', messages.value.length)
    
    // If clicking on the same chat, don't reload messages
    if (selectedChat.value && selectedChat.value.sessionId === chat.sessionId) {
      console.log('⚠️ Same chat selected, skipping reload to prevent duplicates')
      return
    }
    ...
    if (chat.messages && chat.messages.length > 0) {
      console.log('📨 Loading existing messages for chat:', chat.messages.length, 'messages')
      messages.value = [...chat.messages] // Use spread to avoid reference issues
    } else {
      console.log('📨 No existing messages, initializing empty array')
      messages.value = []
    ...
    saveStaffSessionData()
    console.log('✅ Chat selected successfully:', chat.sessionId)
  }
  ...
  const sendCustomerMessage = (message) => {
    if (!socket.value || !isCustomer.value) {
      console.log('❌ Cannot send customer message - socket:', !!socket.value, 'isCustomer:', isCustomer.value)
      return
    }
    
    console.log('📤 Sending customer message:', message, 'sessionId:', sessionId.value)
    ...
    try {
      ...
      console.log('✅ Customer message sent successfully to server')
    } catch (error) {
      ...
      if (customerChatData.value) {
        const customerIndex = customerChatData.value.messages.findIndex(msg => msg.id === messageId)
        if (customerIndex > -1) {
          customerChatData.value.messages.splice(customerIndex, 1)
        }
      }
    }
  }
  ...
  const handleStaffMessage = (data) => {
    ...
    console.log('🔍 DEBUG - Message details:', {
      ...
      selectedChatSessionId: selectedChat.value?.sessionId,
    ...
    switch (data.type) {
      ...
      case 'customer_message':
        ...
        if (selectedChat.value && selectedChat.value.sessionId === data.sessionId) {
          // More strict duplicate check using unique ID if available
          const messageExists = messages.value.some(msg => {
            // Check by content, sender, and timestamp
            const contentMatch = msg.content === data.message
            const senderMatch = msg.sender === 'customer'
            const customerMatch = msg.customerName === data.customerName
            const timeDiff = Math.abs(new Date(msg.timestamp) - new Date(data.timestamp))
            
            return contentMatch && senderMatch && customerMatch && timeDiff < 3000
          })
          ...
          if (!messageExists) {
            ...
            console.log('⚠️ Skipped duplicate customer message:', data.message)
          }
        } else {
          console.log('❌ Message not added - selectedChat:', !!selectedChat.value, 'sessionId match:', selectedChat.value?.sessionId === data.sessionId)
        }
        break
      case 'customer_disconnect':
